...
> **版本**: 3.0 | **日期**: 2026-04-04 | **状态**: 设计中（已修订）
...
┌─────────────────────────────────────────────────────────┐
│                    CommandStack                          │
│  + execute(unique_ptr<Command>, CommandContext&) → CommandResult │
│  + undo(CommandContext&)  + redo(CommandContext&)         │
│  + canUndo()  + canRedo()                                │
│  + openMacro(desc)  + closeMacro()                       │
│  + abortMacro(CommandContext&)                           │
│  + markClean()  + isClean()                              │
│  + setMaxSize(size_t)                                    │
...
│  Signal<string> sceneRemoveRequested    (来自 lastResult_.deletedIds) │
└─────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────┐
│           CommandRegistry (统一工厂注册表)                 │
│  + registerFactory(name, json→unique_ptr<Command>)        │
│  + createFromParams(name, json params) → unique_ptr<Command> │
...
> **修订说明**：移除 `InsertComponent` 枚举值。`InsertComponentCommand` 的 `type()` 统一返回 `Macro`，子类型通过 JSON 的 `"componentType"` 字段区分。新增命令只需注册工厂和 JSON 反序列化函数，无需修改此枚举。`CommandType` 枚举**仅用于** C++ `type()` 虚方法返回值，`CommandRegistry` 的注册 key 统一使用字符串（"SetProperty"、"CreatePipePoint" 等），两套标识系统各有明确职责，不冗余。
...
**修订说明**：移除 `sceneAdd` 和 `sceneRemove`。新增对象的可视化统一通过 `CommandStack::commandCompleted` 信号 → `RecomputeEngine` 路径处理（生成 TopoDS_Shape → 更新 SceneManager）。删除操作的场景节点移除通过命令在 `lastResult_.deletedIds` 中记录，`CommandStack` 在 execute/undo/redo 操作后统一 emit `sceneRemoveRequested` 信号，`main.cpp` 连线到 `sceneManager.removeNode`。`CommandContext` 由 `Application::createCommandContext()` 便利方法构建，调用方（AppController、TableModel）可直接获取完整上下文，无需手动组装。除了拓扑结构命令，任何地方都不需要额外填充 ctx 字段。
...
> - **tryMerge 仅在实时交互模式下调用**：CommandStack 的正向 execute 路径才尝试合并。redo 操作和反序列化防放均跳过 tryMerge。反序列化的命令必须已包含 oldValue，等第于 createWithOldValue，不会再次尝试合并。
...
public:
    ...
    /// @note 必须在主线程调用。execute/undo/redo 均非线程安全。
    ...
    bool isClean() const;        // 当前栈顶命令 UUID 是否等于 clean 时记录的 UUID
...
private:
    ...
    foundation::UUID cleanTopId_;  // markClean 时 undoStack_.back()->id()（isNull = 初始状态为 clean）
...
| 操作 | 行为 |
|------|------|
| `execute(cmd, ctx)` | 若 `macroOpen_`，调用 `cmd->execute(ctx)` 并加入 `pendingMacro_`（不入栈）。否则：尝试 `tryMerge` → `cmd->execute(ctx)` → 入栈 → 清空 redo → trim → 遍历 `lastResult_.deletedIds` emit `sceneRemoveRequested` → emit `commandCompleted(affectedIds)` → emit `stackChanged` |
| `undo(ctx)` | 从 undoStack_ 取出 → `cmd->undo(ctx)`（更新 `lastResult_`）→ 入 redoStack_ → 遍历 `lastResult_.deletedIds` emit `sceneRemoveRequested` → emit `commandUndone(affectedIds)` + `stackChanged` |
| `redo(ctx)` | 从 redoStack_ 取出 → `cmd->execute(ctx)`（更新 `lastResult_`，跳过 tryMerge）→ 入 undoStack_ → 遍历 `lastResult_.deletedIds` emit `sceneRemoveRequested` → emit `commandRedone(affectedIds)` + `stackChanged` |
| `openMacro(desc)` | 创建空 `pendingMacro_`，设 `macroOpen_=true` |
| `closeMacro()` | 设 `macroOpen_=false`，将 `pendingMacro_` 整体推入 undoStack_（子命令已在 execute 阶段执行，不重新执行） |
| `abortMacro(ctx)` | 对 `pendingMacro_` 中已执行的子命令逆序调用 `undo(ctx)`，丢弃 pendingMacro_ |
| `markClean()` | 记录 `cleanTopId_ = undoStack_.empty() ? UUID{} : undoStack_.back()->id()` |
| `isClean()` | `return undoStack_.empty() ? cleanTopId_.isNull() : undoStack_.back()->id() == cleanTopId_` |

> **关于 undo/redo 更新 lastResult_**：`undo()` 执行后命令的 `lastResult_` 被更新，deletedIds 反映此次撤销导致的场景节点删除（例如撤销创建命令 → 删除刚创建的对象）。`redo()` 等同于重新 execute，同样更新 `lastResult_`。各具体命令必须在 `undo()` 中显式设置 `lastResult_.deletedIds` 和 `lastResult_.affectedIds`。

> **clean 标记边界行为**：`trimToMaxSize()` 丢弃最旧命令后，若 `cleanTopId_` 所在的命令已被丢弃，此后 `isClean()` 永远返回 false（正确行为：无法回到 clean 点，除非重新保存）。当用户从 clean 点 undo 再执行新命令（新分支）时，`isClean()` 正确地返回 false，直到用户再次保存和 `markClean()`。

> **线程安全**：`execute`/`undo`/`redo` 必须在主线程调用。OCCT 操作非线程安全。TCP 服务器（Phase 5）通过主线程队列派发，详见 §10.5。
...
public:
    ...
    /// @throws std::invalid_argument 若对象为空或 key 不被该对象类型识别
    static void apply(model::DocumentObject* obj,
                      const std::string& key,
                      const foundation::Variant& value) {
        if (!obj) throw std::invalid_argument("PropertyApplier::apply: null object");
        if (!obj->setProperty(key, value)) {
            throw std::invalid_argument("PropertyApplier::apply: unknown key '" + key +
                                        "' on object '" + obj->name() + "'");
        }
    }
...
当前 `Variant = std::variant<double, int, std::string>`。Phase 1 必须扩展以支持：

| 新增类型 | 用途 | 应对策略 |
|---------|-----|----------|
| `bool` | `PressureLoad::isExternal` | 添加 `bool` 到 variant；目前堆积的 `variantToBool()` hack 不应遗留到新系统 |
| `foundation::math::Vec3` | WindLoad/SeismicLoad/DisplacementLoad 方向向量 | 添加 Vec3 到 variant，统一以 `"windDirection"`、`"seismicDirection"`、`"translation"`、`"rotation"` + Vec3 Variant 表示，废弃拆分 X/Y/Z 属性名 |

若短期内无法扩展 Variant，则可暂时保留拆分分量调用方式，但必须在代码注释中明确标注"待扩展 Variant"而非作为永久方案。
...
struct CommandResult {
    ...
    // 创建的对象快照（供 TCP 客户端确认，仅 Phase 5 启用后填充，早期始终为空）
...
/// @note undo()/redo() 同样更新 lastResult_，命令必须在 execute() 和 undo() 中都正确设置 affectedIds 和 deletedIds
...
public:
    ...
    /// 接收已解析的 params JSON 对象，而非 JSON 字符串（避免二次 parse）
...
execute(ctx):
    ...
    if !obj: throw std::runtime_error("SetPropertyCommand: object not found: " + objectId_.toString())
    if !oldValue_.has_value():           // 首次执行且未提供旧值
        oldValue_ = obj->getProperty(key_)
    if !obj->setProperty(key_, newValue_):
        throw std::invalid_argument("SetPropertyCommand: unknown key '" + key_ + "' on '" + obj->name() + "'")
    lastResult_.success = true
    lastResult_.affectedIds = {objectId_}

undo(ctx):
    obj = ctx.document.findObject(objectId_)
    if !obj: return  // 对象已不存在（对象删除命令的 undo 将先恢复对象）
...
> **createAutoCapture 序列化约束**：`createAutoCapture` 创建的命令在 execute 前 oldValue 为空。序列化时（`toJson()`）必须确保 execute 已调用过，否则抛出异常。反序列化后的命令已包含 oldValue 字段，等价于 `createWithOldValue`，不再走自动捕获路径。

### 3.2 BatchSetPropertyCommand — 批量属性修改

多个属性变更作为一个原子 undo 单元。适用于已知所有变更的程序化批量写入场景（如 UI 表格编辑多列时一次提交）。如果是交互式逐步构建的多步操作（如对话框内复杂编辑），应使用 `openMacro`/`closeMacro` 模式。
...
execute(ctx):
    ...
    8. 设置执行结果：
       ...
       lastResult_.affectedIds = {createdPointId_, 前邻管点ID, 后邻管点ID}
       lastResult_.success = true
...
undo(ctx):
    ...
    5. lastResult_.deletedIds = {createdPointId_}      // CommandStack 将 emit sceneRemoveRequested
    6. lastResult_.affectedIds = {相邻管点 IDs}
    7. lastResult_.success = true
```
...
execute(ctx):
    ...
    6. lastResult_.deletedIds = {pointId_}            // CommandStack 将 emit sceneRemoveRequested
    7. lastResult_.affectedIds = {相邻管点 IDs}
    8. lastResult_.success = true
...
undo(ctx):
    ...
    6. 若 Tee，重建分支段
    7. 恢复附属构件（各附属构件使用 setIdForDeserialization 恢复 UUID）
    8. 显式注册 DependencyGraph 依赖（同 CreatePipePointCommand 步骤 7）
    9. lastResult_.createdIds = {savedState_.id}
       lastResult_.affectedIds = {相邻管点 IDs}
       lastResult_.success = true
```

> **附属构件恢复范围（初始迭代）**：初始实现仅恢复附属构件的基本状态（UUID、名称、类型参数），使用 `setIdForDeserialization` 保证 UUID 不变。附属构件目前不参与几何重算脏传播，因此不重新注册 DependencyGraph 依赖。复杂附属构件层次结构的恢复在后续迭代中完善。
...
CommandStack::execute(unique_ptr<Command>)
  │
  ├── 尝试 tryMerge（连续相同属性变更合并为一条命令；redo 路径跳过）
  ├── cmd->execute(ctx)          // 命令执行实际变更，更新 cmd->lastResult_
  ├── undoStack_.push(cmd)
  ├── redoStack_.clear()
  ├── trimToMaxSize()
  ├── 遍历 lastResult_.deletedIds → emit sceneRemoveRequested(uuid)  // 立即移除场景节点
  ├── emit commandCompleted(lastResult_.affectedIds)  // ← 人传播 + recompute
  └── emit stackChanged                               // UI 刷新 canUndo/canRedo
```

**main.cpp 信号连线**（脂传播 + 场景更新）：
...
```
CommandStack::undo(CommandContext& ctx)   // ctx 由调用方（AppController）构建并传入
  │
  ├── cmd = undoStack_.pop_back()
  ├── cmd->undo(ctx)                // 反向恢复，更新 cmd->lastResult_
  ├── redoStack_.push(cmd)
  ├── 遍历 lastResult_.deletedIds → emit sceneRemoveRequested(uuid)
  ├── emit commandUndone(lastResult_.affectedIds)  // ← 信号：触发脏传播+recompute
  └── emit stackChanged
```
...
2. AppController:
   - `auto ctx = Application::instance().createCommandContext()`
   - commandStack_.undo(ctx)
3. CommandStack::undo(ctx):
...
5. CreatePipePointCommand::undo(ctx):
   ...
   - lastResult_.deletedIds = {createdPointId_}    // 不再调用 ctx.sceneRemove
   - lastResult_.affectedIds = {相邻管点 IDs}
6. CommandStack:
   - emit sceneRemoveRequested(createdPointId_)    // ← 来自 lastResult_.deletedIds
   - emit commandUndone(affectedNeighborIds)
   → main.cpp 信号处理器：
...
复用 `ProjectSerializer` 中已有的 `variantToJson()`/`jsonToVariant()` 辅助函数，Phase 1 扩展后支持以下格式：
...
Phase 1 将 `bool` 和 `foundation::math::Vec3` 添加到 Variant 并同步扩展序列化辅助函数。
...
public:
    ...
    /// 便利方法：构建 CommandContext。所有调用点（AppController、TableModel等）均通过此方法获取上下文。
    /// 返回的 ctx 无需额外填充字段（sceneRemove 已移除，节点删除通过 lastResult_.deletedIds 信号处理）。
    app::CommandContext createCommandContext() {
        return app::CommandContext{
            .document = *document_,
            .dependencyGraph = *dependencyGraph_,
            .topologyManager = topologyManager_.get()
        };
    }
...
commandStack.sceneRemoveRequested.connect([&](const auto& uuid) {
    // 来源：CommandStack 在 execute/undo/redo 后从 lastResult_.deletedIds 提取并 emit
    sceneManager.removeNode(uuid);
});
...
| `undo()` | `transactionManager_.undo()` | `auto ctx = Application::instance().createCommandContext(); commandStack_.undo(ctx)` |
| `redo()` | `transactionManager_.redo()` | `auto ctx = Application::instance().createCommandContext(); commandStack_.redo(ctx)` |
| `canUndo()` | `transactionManager_.canUndo()` | `commandStack_.canUndo()` |
| `insertComponent()` | `emit insertComponentRequested(type)` | `auto ctx = Application::instance().createCommandContext()` + `InsertComponentCommand` + `commandStack_.execute(cmd, ctx)` |
| `deleteSelected()` | `emit deleteRequested(uuids)` | `auto ctx = Application::instance().createCommandContext()` + `DeletePipePointCommand` + `commandStack_.execute(cmd, ctx)` |
...
- `AppController::undo()`/`redo()` 改为调用 `commandStack_.undo(ctx)`/`redo(ctx)`
- `main.cpp` 连线 CommandStack 信号到 DependencyGraph + RecomputeEngine + SceneManager
- `TransactionManager` 不再被任何 UI 代码调用
- **此阶段尚不包含** `insertComponent` / `deleteSelected` 迁移（这两个依赖结构命令，属于 Phase 3）
- **验证点**：所有现有测试继续通过

### Phase 3 — 结构命令 + 插入/删除迁移

- 实现 `CreatePipePointCommand`、`DeletePipePointCommand`
- 实现 `InsertComponentCommand`
- `AppController::insertComponent()` 迁移到 `InsertComponentCommand` + `commandStack_.execute(cmd, ctx)`
- `AppController::deleteSelected()` 迁移到 `DeletePipePointCommand` + `commandStack_.execute(cmd, ctx)`
- 注册结构命令到 `CommandRegistry`
...
private:
    ...
    // 内部调用 registry_.createFromParams(name, paramsJson已解析) + commandStack_.execute(cmd, ctx)
};
```
...
| **移除 `sceneAdd` 和 `ctx.sceneRemove`** | 删除操作通过 `lastResult_.deletedIds` 通知，由 CommandStack 统一 emit `sceneRemoveRequested`，单一路径无歧义 |
...
| **`cleanTopId_` UUID 跟踪 clean 点** | 用栈顶命令 UUID 取代栈深度索引，trim 后不失效，分支切换后正确标 dirty |
| **execute/undo/redo 接受 ctx 参数** | 调用方构建 ctx（通过 `Application::createCommandContext()`），CommandStack 不持有任何业务引用 |
| **tryMerge 仅实时交互路径** | 防止反序列化/redo 路径意外合并命令 |
| **`PropertyApplier::apply` 抛异常** | 未知 key 立即显式报错，而非静默忽略 |
| **`Application::createCommandContext()`** | 统一构建入口，避免各调用点手动组装三个引用 |
...
| # | 问题 | 修订内容 | 章节 |
|---|------|---------|------|
| 1 | execute/undo/redo 缺少 CommandContext 参数 | CommandStack API 全加 ctx 参数；`execute(cmd, ctx)`/`undo(ctx)`/`redo(ctx)`/`abortMacro(ctx)` | §2.1, §2.6, §4 |
| 2 | Variant 类型不足（缺 bool/Vec3） | Phase 1 扩展 Variant；添加 bool 和 Vec3；序列化格式数目相应增加 | §2.7, §5.5 |
| 3 | openMacro 模式中 ctx 生命周期不明确 | execute 在 macro 模式下同样传入 ctx；closeMacro 无需 ctx；abortMacro 需 ctx | §2.6(行为表) |
| 4 | sceneRemove 双路径冗余（ctx.sceneRemove vs CommandStack信号） | 移除 ctx.sceneRemove；命令通过 lastResult_.deletedIds 通知；CommandStack 统一 emit sceneRemoveRequested | §2.1, §2.3, §2.6, §3.3, §3.4, §4 |
| 5 | cleanIndex_ 在 trim 后失效 | 改用 `cleanTopId_`（栈顶命令 UUID）跨越 trim、分支均正确 | §2.6 |
| 6 | tryMerge 在反序列化/redo 场景行为不可预测 | 明确 tryMerge 仅在实时交互的正向 execute 路径中调用 | §2.4, §3.1 |
| 7 | setProperty 返回值未检查，静默失败 | PropertyApplier::apply 检查返回值，失败时抛 std::invalid_argument | §2.7 |
| 8 | 缺少构建 CommandContext 的便利方法 | Application::createCommandContext() | §6.1 |
| 9 | Phase 2 包含结构命令，但结构命令依赖 Phase 3 | Phase 2 仅迁移属性层，Phase 3 包含结构命令 + insertComponent/deleteSelected 迁移 | §8 |
| 10 | createFromParams 参数为 string 导致二次 parse | 改为接受 `const nlohmann::json& params` | §2.9 |
| 11 | createdObjects 在早期填充浪费内存且无用 | 明确标注“Phase 5 前始终为空” | §2.8 |
| 12 | 附属构件 undo 恢复范围不明确 | 添加初始迭代范围说明：UUID 保留，不重注册 DependencyGraph，层次恢复后续完善 | §3.4 |
| 13 | createAutoCapture 序列化时 oldValue 可能为空 | 明确序列化前必须已 execute；反序列化后等价于 createWithOldValue | §3.1 |
| 14 | 缺少错误处理策略说明 | 添加语义：execute/undo 抛异常，CommandStack 不入失败命令，信号仅在成功后 emit | §2.6(行为表备注) |
| 15 | CommandType 枚举与字符串 key 冗余 | 明确枚举仅用于 C++ type() 返回值，注册表统一使用字符串 key | §2.2 |
| 16 | BatchSetProperty 与 openMacro 选择标准不明 | 添加说明：已知所有变更用 Batch，通过交互逐步构建用 openMacro | §3.2 |
| 17 | 线程安全仅在 TCP 章节提及 | 在 CommandStack API 和行为表加强“必须主线程调用”注释 | §2.6 |
