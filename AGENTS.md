...
- Phase 4 (T50-T77): 🔄 进行中 — lib/apps 架构重构、异步重算管线

**当前实现同步**:
- `TransactionManager` 已从代码库移除，编辑流程统一走 `CommandStack` + `CommandRegistry` + `CommandContext`
- `src/command/` 源文件已并入 `app` 静态库；`command` target 保留为指向 `app` 的 INTERFACE 别名
- 正在进行 Phase 4 架构重构：`src/lib/` 新目录结构已建立，`src/apps/pipecad/` 承载业务代码
- 当前基线：编译通过 208/208，测试通过 42/42
...
```bash
./build/debug/src/pipecad          # Debug 版本
./build/release/src/pipecad        # Release 版本
```

### 并发控制

```bash
# 环境变量控制并发（默认：编译=6，测试=1）
...
项目采用 **8 层工程分层（7 个核心库层 + 1 个应用入口层）**，核心静态库显式依赖:

```
foundation → geometry → model → engine → visualization → app → ui → pipecad
               └──→ vtk-visualization ──────────↑
```

| 层 | 目录 | 职责 |
|----|------|------|
| Layer 1: Foundation | `src/foundation/`, `src/lib/base/foundation/` | 基础类型: UUID、Variant、Math、Signal、Log |
| Layer 2: Geometry | `src/geometry/`, `src/lib/platform/occt/geometry/` | OCCT 封装: ShapeBuilder、BooleanOps、StepIO、ShapeMesher |
| Layer 3: Model | `src/model/`, `src/apps/pipecad/model/` | 文档模型: PipePoint、PipeSpec、Segment、Route、Load 层次结构 |
| Layer 4: Engine | `src/engine/`, `src/apps/pipecad/engine/` | 管道领域引擎: ComponentCatalog、BendCalculator、各 Builder、RecomputeEngine |
| Layer 5: Visualization | `src/visualization/`, `src/lib/platform/vsg/visualization/` | VSG 渲染: OcctToVsg、SceneManager、PickHandler、ViewManager |
| Layer 5b: VTK Visualization | `src/vtk-visualization/`, `src/lib/platform/vtk/vtk-visualization/` | VTK 分析视图: OcctToVtk、VtkSceneManager、VtkViewport |
| Layer 6: Application | `src/app/`, `src/lib/framework/app/`, `src/lib/runtime/app/` | 应用层: Document、Workbench 系统、ProjectSerializer、SelectionManager、CommandStack、CommandRegistry、TaskQueue |
| Layer 7: UI | `src/ui/`, `src/apps/pipecad/ui/` | QML 桥接: VsgQuickItem、VtkViewport、Table/Tree Models、AppController |
| Layer 8: App Entry | `src/main.cpp`, `src/apps/pipecad/main.cpp` | 初始化 Application、注册命令工厂、连线 CommandStack 与 RecomputeEngine、加载 QML |
...
├── model/                        # 旧目录 (兼容别名)
├── engine/                       # 旧目录 (兼容别名)
├── visualization/                # 旧目录 (兼容别名)
├── vtk-visualization/            # 旧目录 (兼容别名)
├── app/                          # 旧目录 (兼容别名)
├── command/                      # INTERFACE 别名指向 app
├── ui/                           # 旧目录 (兼容别名)
└── main.cpp                      # 旧入口 (转发到 apps/pipecad)
```
...
- **命令系统**: `CommandStack` 统一执行/撤销/重做，`CommandRegistry` 负责工厂与序列化，命令完成后驱动重算
- **DocumentSnapshot**: 文档只读快照，用于后台任务并发访问
...
### 模块接口单元 (C++20 Modules, Phase 4 新增)

```cpp
// src/lib/<layer>/xxxMod/pipecad.xxx.yyy.cppm
export module pipecad.xxx.yyy;
export import :snapshot;  // 子模块
```
...
- 命名: 测试目标名与源文件名一致 (如 `test_foundation`)
...
- 每个 Task 必须包含对应单元测试
- 集成测试覆盖跨层联动场景
- 当前基线以 `docs/tasks/status.md` 为准；测试基线 42/42
...
```
读取 docs/tasks/phase4-lib-app-refactor/ 下对应任务卡（Phase 4 任务）
...
3. **推荐模型**（Opus 4.6 / Sonnet 4.6 / Gemini 3.1 Pro / GPT 5.3 Codex / GPT-5.4 Codex）
4. **需要读取的文件列表**（精确到文件路径）
...
| `docs/archive/task-specs/command-pattern-design.md` | 命令模式架构设计 v3.0（Phase 3，已归档） |
| `docs/archive/task-specs/development-plan.md` | Phase 1/2 任务详情（交付物、验收标准、依赖关系，已归档） |
| `docs/tasks/phase4-lib-app-refactor/` | Phase 4 任务卡目录（lib/apps 架构重构） |
| `docs/archive/task-logs/t01-t25.md` | Phase 1 完成记录（T01–T25，已归档） |
| `docs/archive/task-logs/t30-t45.md` | Phase 2 完成记录（T30–T45，已归档） |
| `docs/archive/task-logs/command-pattern.md` | Phase 3 完成记录（命令模式 T0–T10，已归档） |
| `docs/archive/task-logs/phase4-refactor.md` | Phase 4 完成记录（T50–T77，更新中） |
...
10. **编译/构建/运行/测试命令的输出必须重定向到临时目录**：执行任何构建、测试、运行命令前，先在项目根目录下创建临时目录（如 `tmp_build_logs/`），将命令输出重定向到该目录下的文件，阅读完毕后立即删除文件和目录，避免大量输出内容堆积消耗 token
11. **禁止操作项目目录以外的任何文件或目录**：所有读写操作必须限定在当前项目目录（`/home/zcli/code/simplePIPE/`）内，严禁向任何外部路径（包括 `~/`、`/tmp/`、其他项目目录等）读取或写入内容
12. **Phase 4 架构重构期间注意**: 新代码优先放入 `src/lib/` 或 `src/apps/` 对应目录，旧目录仅保留兼容别名
...
- `lib/occt/` - OpenCASCADE 8.0.0 预编译库（需手动放置）
- `lib/vsg/` - VulkanSceneGraph 1.1.13 预编译库（需手动放置）
- VTK 已从 `lib/vtk/` 迁移至 pixi conda-forge 管理
