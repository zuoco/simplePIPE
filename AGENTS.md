...
7. **任务完成后必须立即更新状态**：每完成一个任务，必须在 `status.md` 中将状态改为 `done`、填写完成日期、追加完成记录、更新后续任务状态、更新接力文件 `current.md`，**严禁遗漏**
8. **遇到无法修复的故障必须立即停止**：当出现无法自行修复的编译错误、运行时崩溃、环境问题或其他阻塞性故障时，必须：
   - 立即停止当前任务的进一步开发
   - 在 `status.md` 中将任务状态更新为 `blocked`，并在完成记录区域追加故障详情（错误信息、复现步骤、已尝试的修复方案）
