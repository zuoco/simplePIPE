...
ALL_LIBS_OK=true
for LIB_NAME in occt vsg vtk; do
...
if [ "${VERIFY_ONLY}" = true ]; then
    ...
    if pixi run configure-debug; then
        ...
        warn "常见原因: 本地预编译库 (lib/occt, lib/vsg, lib/vtk) 缺失"
...
echo -e "  工具链: pixi + CMake + Ninja, C++17"
echo ""
...
if [ "${ALL_LIBS_OK}" = true ]; then
    ...
    warn "请将 OCCT/VSG/VTK 预编译库放置到 lib/ 目录后重新运行本脚本"
fi
